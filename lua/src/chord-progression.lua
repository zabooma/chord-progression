...
function factory ()
    ...
    function print_table(tbl, prefix)
        local result = {}
        prefix = prefix or ""  -- Default to an empty string if no prefix is provided
        ...
        -- If it's a list, print the values
        if is_list then
            for _, v in ipairs(tbl) do
                table.insert(result, tostring(v))
            end
        else
            -- Otherwise, print key-value pairs
            for k, v in pairs(tbl) do
                table.insert(result, tostring(k) .. ": " .. tostring(v))
            end
        end

        print(prefix .. "{" .. table.concat(result, ", ") .. "}")
    end
    ...
    function choose_inversion(chord_str, previous_inversion, previous_chord, previous_octave_adjustment)
        ...
        if previous_chord then
            ...
            print_table(prev_notes, "Previous chord notes")
            ...
            for octave_adjustment = -1, 1 do
                for inversion = 0, #chord_type_map[chord_type] - 1 do
                    ...
                    print_table(candidate_notes, "Candidate notes, inversion " .. tostring(inversion) .. " octave adjustment " .. tostring(octave_adjustment))
            ...
            print ("Choosing inversion " .. tostring(best_inversion) .. " octave adjustment " .. tostring(best_octave_adjustment))
    ...
    function parse_chord_progression_config(str)
        ...
        for key, values in content:gmatch("([%w_]+)%(([^%)]*)%)") do
            ...
            print_table(value_table, "Config for " .. key)
        end
                ...
                for _, hand in ipairs({1, 2}) do
                    ...
                    for i, marker in ipairs(relevantChordMarkers) do
                        ...
                        print("Adding chord ", chordStr, " at", start_time, " with duration ", duration,
                                hand, " inversion ", inversion, " octave adjustment ", octave_adjustment)

                        -- Add chord to MIDI for the current hand
                        add_chord_to_midi(midiCommand, chordStr, inversion, octave,
                                start_time, duration,
                        ...
                        -- Save current chord string for the next iteration
